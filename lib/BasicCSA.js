 ...
 */
var ResultStream = function (config, query) { //TODO: , footpathsTo) {
  ...
  //prefetch wheelchair accessible data if necessary
  this._no_wha_stop_ids = [];
  if(this._wheelchairAccessible){
    //prefetch trips data
...
ResultStream.prototype._transform = function (connection, encoding, done) {
  ...
  if (this._earliestArrivalTimes[departureStop] && addSeconds(this._earliestArrivalTimes[departureStop].arrivalTime,getTransferTime(this,connection)) <= connection.departureTime) {
 ...
 */
function getTransferTime(_this, connection) {
  var perviousConnection = _this._earliestArrivalTimes[connection["departureStop"]];
  ...
  if(perviousConnection && perviousConnection["trip"]!=null && perviousConnection["trip"] != connection["gtfs:trip"]["@id"]) {
    if(_this._wheelchairAccessible) {
      //check if stop is wheelchair accessible
      if(_this._no_wha_stop_ids.indexOf(connection["departureStop"]) > -1) {
        return TRANSFER_MAX;
      } else {
        _this._minTransferTime*2;
      }
    } else {
      //TODO: transfertime from gtfs
      return _this._minTransferTime;
    }
  }
  return 0;
}
