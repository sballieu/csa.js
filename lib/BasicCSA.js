 ...
 */
var ResultStream = function (query, minimumTransferTimes) { //TODO: , footpathsTo) {
...
ResultStream.prototype._transform = function (connection, encoding, done) {
  ...
  //var test = this._getTransferTime(null,null);
  ...
  var transferTime = this._minimumTransferTimes.getTransferTime(this._earliestArrivalTimes[connection["departureStop"]],connection,this._minTransferTime,this._wheelchairAccessible);

  if (this._earliestArrivalTimes[departureStop] && addSeconds(this._earliestArrivalTimes[departureStop].arrivalTime,transferTime) <= connection.departureTime) {
